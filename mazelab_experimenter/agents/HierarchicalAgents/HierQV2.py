...
class TabularHierarchicalAgentV2(TabularHierarchicalAgent, ABC):
    ...
    def __init__(self, observation_shape: typing.Tuple, n_actions: int, n_levels: int,
                 ...
                 legal_states: np.ndarray = None, flat_training: bool = False, **kwargs) -> None:
        ...
        # Only atomic exploration by default10k for a given epsilon.
        ...
        self.flat_training = flat_training
        ...
        for i in range(self.n_levels):
            critic = CriticTable(0.0, (len(self.S), len(self.G[i]), len(self.A[i])),
                                 goal_conditioned=(i < self.n_levels - 1))
            critic.reset()
...
class HierQV2(TabularHierarchicalAgentV2):
    ...
    def get_level_action(self, s: int, g: int, level: int, explore: bool = True) -> typing.Tuple[int, float]:
        ...
        if not level:
            ...
            a = rand_argmax(self.critics[level].table[s, gc] * int(eps < np.random.rand()))
            return a, self.critics[level].table[s, gc, a]

        if explore and self.flat_training:  # Simply follow the end-goal.
            return g, 1.0

        greedy = int((not explore) or (np.random.rand() > self.epsilon[level]))

        pref = g  # Goal-selection bias for shared max Q-values.
    ...
    def update_table(self, level: int, horizon: int, trace: typing.Sequence[GoalTransition], **kwargs) -> None:
        ...
        for h in reversed(range(horizon)):
            ...
            if level:  # Convert goal-action 'a' from Absolute-to-Neighborhood.
            ...
            ys = mask + (1 - mask) * self.discount * bootstrap
    ...
    def update(self, _env: gym.Env, level: int, state: int, goal_stack: typing.List[int],
        ...
        while (step < self.horizons[level]) and (state not in goal_stack) and (not done):
            ...
            if level:  # Temporally extend action as a goal through recursion.
                ...
                for i in range((self.n_levels if not raw else 1)):
                    t = self.trace if i else self.trace.raw
                    self.update_table(level=i, horizon=self.trace.window(i, raw=(i == 0)), trace=t)
