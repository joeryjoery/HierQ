...
class HierQ(TabularHierarchicalAgent):
    ...
    def get_level_action(self, s: int, g: int, level: int, explore: bool = True) -> typing.Tuple[int, float]:
        ...
        a = rand_argmax(self._q_tables[level][s][g] * greedy, preference=(g if level else None))
        return a, self._q_tables[level][s][g][a]
    ...
    def train(self, _env: gym.Env, num_episodes: int, progress_bar: bool = False, **kwargs) -> None:
        ...
        for _ in iterator:
            ...
            while (not done) and (state != goal_stack[0]):
                ...
                a, v = self.get_level_action(s=state, g=goal_stack[-1], level=self.n_levels - 1)
